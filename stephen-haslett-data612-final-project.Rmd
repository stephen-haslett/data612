...
```{r, message=FALSE, warning=FALSE}
names(amazon_magazine_metadata)
```

Ratings column names.
```{r, message=FALSE, warning=FALSE}
names(amazon_magazine_ratings)
```
...
Because The dataset is too large for my local machine to handle, we need to reduce the number of records to send over to Spark.
To accomplish this, we will select the first 2000 records.
```{r, message=FALSE, warning=FALSE}
