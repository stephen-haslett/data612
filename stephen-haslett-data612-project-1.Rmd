...
```{r, warning=FALSE, message=FALSE}
user_bias <- rowMeans(train_data, na.rm = TRUE) - training_average_mean
user_bias %>% kable(caption = 'User Bias') %>% kable_styling("striped", full_width = TRUE)
```


```{r, warning=FALSE, message=FALSE}
item_bias <- colMeans(train_data, na.rm = TRUE) - training_average_mean
...
colnames(base) <- colnames(user_item_matrix)
kable(round(base, 2))
```
