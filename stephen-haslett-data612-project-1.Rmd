...
### Training Baseline Predictors RMSE
```{r, warning=FALSE, message=FALSE}
training_baseline_rmse <- sqrt(mean((as.matrix(train_data - baseline)) ^2, na.rm = TRUE))
training_baseline_rmse
```


### Testing Baseline Predictors RMSE
```{r, warning=FALSE, message=FALSE}
test_baseline_rmse <- sqrt(mean((as.matrix(test_data - baseline)) ^2, na.rm = TRUE))
test_baseline_rmse
```
